...
export function CompanyInvoicesTable({ companyId }: CompanyInvoicesTableProps) {
    ...
    const pageSize = 5;
    const { companyInvoices, isLoading, error, getCompanyInvoices } =
        useCompany();
    const [totalPages, setTotalPages] = useState(1);
    const [searchQuery, setSearchQuery] = useState("");
    useEffect(() => {
        try {
            getCompanyInvoices(companyId.toString(), { page, pageSize });
            setTotalPages(Math.ceil(companyInvoices.length / pageSize));
        } catch (error) {
            console.error(error);
