...
export function LoginForm() {
  ...
  return (
    <div className="w-full bg-white rounded-[32px] px-8 py-8 max-h-[calc(100dvh-24px)] overflow-y-auto shadow-2xl shadow-blue-900/20 animate-in fade-in zoom-in-95 duration-700">

      <div className="text-center mb-6">
        <div className="flex justify-center mb-5">
          <img src="/logo-hitradies-standard-04.png" alt="Logo" className="h-[72px] w-auto object-contain" />
        </div>
        ...
        <p className="text-[13.5px] text-slate-500 font-medium font-sans">Internal system for HiTradies administrators</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-3.5">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between ml-1">
            ...
            title="Quên mật khẩu" 
            className="text-[12px] hover:text-slate-900 hover:underline cursor-pointer transition-colors">
              Forgot password?
            </Link>
          </div>
        ...
        >
          {isLoading ? <Loader2 className="animate-spin mr-2 w-5 h-5" /> : "Login"}
        </Button>

        {authError && !isEmailError && !isPasswordError && (
           <p className="text-[12.5px] text-red-500 text-center font-bold mt-2">{authError}</p>
        )}
      </form>
      ...
      <Button 
        type="button"
        variant="outline" 
        className="w-full h-11 border-slate-200 rounded-xl font-bold text-slate-700 hover:bg-slate-50 relative shadow-sm cursor-pointer transition-all active:scale-[0.98]"
      >
        <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" width="18" height="18" alt="Google" className="absolute left-7" />
        <span className="text-[13px]">Đăng nhập bằng Google</span>
      </Button>
