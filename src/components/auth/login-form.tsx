...
export function LoginForm() {
  ...
  return (
    /* Wrapper bao ngoài để căn giữa form trên toàn màn hình */
    <div className="flex items-center justify-center min-h-[100dvh] w-full p-4 bg-slate-50/50">
      
      {/* Box trắng chính của Form */}
      <div className="w-full max-w-[440px] bg-white rounded-[32px] px-8 py-10 shadow-2xl shadow-blue-900/10 animate-in fade-in zoom-in-95 duration-700">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between ml-1">
              ...
              <Link
                ...
                className="text-[12px] text-slate-500 hover:text-[#0066FF] hover:underline transition-colors"
              >
                Forgot password?
              </Link>
            </div>
          ...
          <Button
            type="submit"
            disabled={isLoading}
            className="w-full h-11 bg-[#00A3FF] hover:bg-[#0086d1] text-white font-bold rounded-xl shadow-lg shadow-blue-100 transition-all mt-2 active:scale-[0.98] disabled:opacity-70"
          >
        ...
        <Button
          ...
          variant="outline"
          disabled={isLoading}
          className="w-full h-11 border-slate-200 rounded-xl font-bold text-slate-700 hover:bg-slate-50 relative shadow-sm transition-all active:scale-[0.98] disabled:opacity-70"
        >
          <img
        ...
        <div className="text-center mt-6 text-[13.5px] text-slate-500">
          ...
          <Link
            href="/register"
            className="font-bold text-slate-900 underline underline-offset-4 hover:text-[#00A3FF] transition-colors"
          >
            Sign up
          </Link>
        </div>
